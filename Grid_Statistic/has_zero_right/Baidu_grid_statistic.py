...
def get_info_set(now_class,x):
	...
	cursor = conn.cursor()
	info = cursor.execute('''SELECT poiid,lng,lat,%s FROM %s
							WHERE (lng >= %f AND lng <= %f) AND (lat >= %f AND lat<= %f) 
							'''%(now_class,city_name,x[0],x[1],x[2],x[3]))
	ret = []
	for row in info:
		#print(row)
		#print("poiid: %s | lng: %f| lat: %f |class: %d"%tuple(row))
...
for class_i in range(len(sem_class)):
	...
	for i in range(len(center_set)):
		...
		if(len(now_info_set)==0):
			class_data[i]=[int(i%x_len),int(i//x_len),0]
		else:
			class_data[i]=[int(i%x_len),int(i//x_len),sum(now_info_set)/len(now_info_set)]
	#feature scaling
	...
	for i in range(len(class_data)):
		...
		if(i != len(class_data)-1):
			class_data_list.append([int(i%x_len),int(i//x_len),class_data[i][2] ])
			grid_data.write("[%d,%d,%f],"%(int(i%x_len),int(i//x_len),class_data[i][2]))
			print("[%d,%d,%f],"%(int(i%x_len),int(i//x_len),class_data[i][2]),end="")
		else:
			grid_data.write("[%d,%d,%f]"%(int(i%x_len),int(i//x_len),class_data[i][2]))
			print("[%d,%d,%f]"%(int(i%x_len),int(i//x_len),class_data[i][2]),end="")
	grid_data.write("]\n")
	print("]")
	if(class_i != len(sem_class)-1 ):
		grid_data.write(",\n")
		print(",")
