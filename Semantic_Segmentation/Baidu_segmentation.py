...
if __name__ == '__main__':
    ...
    seg_log = catch_log_file.read().strip().split("\n")
    seg_log = set(seg_log)
    seg_log_file.close()
    ...
    for i in poi_set:
        if(i in seg_log):
            continue
        for j in range(4):
            args.input_image="%sBaidu_Streetview_Pictures/%s/%s_%d.jpg"%(root_path,args.cityname,i,j*90)
            ...
            print("City: %10s || Total: %5d || Now: %5d || Percent: %.3f || Now_Pic: %s_%d.jpg"%(args.city_name,len(poi_set),len(log_set),1.0*len(log_set)/len(input_files),i,j) )
