...
for i in range(last_try,len(point_set)):
	...
	if(ret['status']==0):
		...
		for poi_index in range(len(pois)):
			...
			multi_temp_arg = []
			for j in range(4):
				multi_temp_arg.append(copy.deepcopy(temp_arg))
				multi_temp_arg[j]['heading'] = j*90

			process = Pool(4)
			save_result=[]

			for j in range(4):
				#print(multi_temp_arg[j])
				#multi_temp_arg = temp_arg
				#multi_temp_arg['heading'] = j*90
				img_path = '%sBaidu_Streetview_Pictures/%s/%s_%d.jpg'%(root_path,city_name,poi_id,j*90)
				save_result.append( process.apply_async(save_picture, args=(multi_temp_arg[j],img_path,)) )
